 ...
 ************************************************************************************/
Settings_Vtiger_Edit_Js("Settings_EERainbowList_Edit_Js",{
    ...
    /**
     * Function to get the container which holds all the rainbow elements
     ...
     * @params : element - which represents the rainbow container
     * @return : current instance
     */
    setContainer : function(element) {
        this.rainbowContainer = element;
        return this;
    },


    /**
     * Function to return the instance based on the step of the rainbow
     */
    getInstance : function(step) {
        if(step in Settings_EERainbowList_Edit_Js.instance ){
            return Settings_EERainbowList_Edit_Js.instance[step];
        } else {
            var moduleClassName = 'Settings_EERainbowList_Edit'+step+'_Js' ;
            Settings_EERainbowList_Edit_Js.instance[step] =  new window[moduleClassName]();
            return Settings_EERainbowList_Edit_Js.instance[step]
        }
    },
    ...
    initiate : function(container){
        if(typeof container == 'undefined') {
            container = jQuery('.rainbowContents');
        }
        if(container.is('.rainbowContents')) {
            this.setContainer(container);
        }else{
            this.setContainer(jQuery('.rainbowContents',container));
        }
        this.initiateStep('1');
    ...
    back : function(){
        ...
        var container = this.currentInstance.getContainer();
        var rainbowRecordElement = jQuery('[name="record"]',container);
        var rainbowId = rainbowRecordElement.val();
        container.remove();
        this.initiateStep(prevStep);
        var currentContainer = this.currentInstance.getContainer();
        currentContainer.show();
        jQuery('[name="record"]',currentContainer).val(rainbowId);
        var modulesList = jQuery('#moduleName',currentContainer);
        if(modulesList.length > 0 && rainbowId != '') {
